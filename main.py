...
class AddressBookApp:
    ...
    def show_contacts(self):
        self.view.display_contacts(self.book.data.values())
...
class Notes(UserList):
    ...
    def sort_by_date(self):
        if self:
            sorted_by_date = sorted(self, key=lambda note: datetime.strptime(
                note.data["date"], "%d.%m.%Y %H:%M:%S"))
    ...
    def sort_by_date_reverse(self):
        if self:
            sorted_by_date_reverse = sorted(self,
                                            key=lambda note: datetime.strptime(
                                                note.data["date"], "%d.%m.%Y %H:%M:%S"),
                                            reverse=True)
...
def find_note(name=None):  # Пошук нотатки за ключовими словами
    unique_tags = set(
        tag for note in my_notes for tag in note.data.get("tags", []))
...
# https://github.com/Reacross/Angry_Racoons/blob/main/main.py
